...
const DownloadScheduleButton: React.FC<DownloadScheduleButtonProps> = ({ schedule }) => {
  ...
  return (
    <PDFDownloadLink
      ...
      className="text-md text-white bg-green-600 hover:bg-green-700 border-0 rounded-md hover:scale-105 font-semibold duration-75 p-1.5 shadow-sky-800 shadow-md flex items-center" title="Baixar Escala"
    >
      {({ loading }) => (loading ? 'Gerando PDF...' : (
          <span className="flex items-center">
            <FaDownload className="w-3.5 h-3.5 mx-0.5" />
            <span className="w-fit justify-center ml-1 hidden sm:block">Baixar Escala</span>
          </span>
        ))}
    </PDFDownloadLink>
  );
